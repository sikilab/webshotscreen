...
async function drawTile(ctx, image, tile, metrics, exportRect, pixelRatio, clampScale, masterCanvas) {
  ...
  const duplicateHeaderCropTop = Math.max(0, Math.round(tile.duplicateHeaderCropTop || 0));
  const tileRect = {
    left: tile.x,
    top: tile.y + duplicateHeaderCropTop,
    right: tile.x + tileWidth,
    bottom: tile.y + tileHeight,
    width: tileWidth,
    height: Math.max(0, tileHeight - duplicateHeaderCropTop)
  };
