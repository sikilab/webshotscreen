(() => {
  ...
  const state = {
    prepared: false,
    options: { scrollTarget: "right", hideFixedElements: true },
    windowScrollX: 0,
    windowScrollY: 0,
    elementScrollLeft: 0,
    elementScrollTop: 0,
  ...
  chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    ...
    switch (message?.type) {
      ...
      case "FULLPAGE_SCROLL_TO":
        scrollToPosition(message.x, message.y).then(ok).catch(fail);
        return true;
  ...
  async function scrollToPosition(x = 0, y = 0) {
    ...
    return { ...metrics, x: metrics.scrollX, y: metrics.scrollY };
  }
  ...
  function resolveOptions(options = {}) {
    ...
    return { scrollTarget, hideFixedElements: options.hideFixedElements !== false };
  }
